...
module.exports = function(app) {
  ...
  controller.salvar = function(req, res, next) {
    ...
    if (_body._id) {
      ...
      if (!_body.grupo) {
        var _grupo = app.models.grupo;
        var _grupoBody = {};
        _grupo.create(_grupoBody)
          .then(function(pObjeto){
            _body.grupo = pObjeto._id;
            
            //TODO: fix duplicate code
            _objeto.create(_body)
            .then(function(pObjeto) {
                req.body.senha = plainPass;
                app.services.auth.login(req, res, next);
              },
              function(erro) {
